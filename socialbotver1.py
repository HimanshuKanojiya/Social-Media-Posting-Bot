...
class Social_bot:
    ...
    def page_load(self, path = None):
        #This function is to load page, sucessful operation will return None
        ...
        if path == None:
            ...
            if self.browser_session == None:
                ...
                #Sucessful operation will also add one in instance variable to track number of visited pages
                self.browser_session.get(path)
                self.browser_visit+=1
    
    def reverse_visit(self, back_v = None):
        #This function is to go back to previous page
        #This function takes argument back_v for telling function to back number of pages
        ...
        if self.browser_visit == None:
            ...
            #if back_v is not given, then with help of loop will go back
    ...
    def do_login(self, user_name = None, user_pass = None):
        #This function is to do login on social media, takes two arguments user name and password
        #By default these argument set to None, and in no use
        
        if self.browser_session == None or self.login == 1:
            #if self.browser not started, or already login happend then it will return -1
            return -1
            ...
            #Sucessful operations will use xpath, and login to use the site
    ...
    def do_logout(self):
        #This function is to use log out the social media
        
        if self.browser_session == None or self.login == 0:
            #if no session available or login then it will return -1
            return -1
        
        else:
            #To logout the site via xpath, and also follow time_patterns() function to pause the 
            #script for sometime.
            #Important bcz sometimes some xpath are not loaded, and take time to load, so this function is important
            #Failed operation will return -1, and sucessful will return 1
            try:
                self.browser_session.find_element_by_xpath(self.logout_fb[0]).click()
                self.time_patterns()
                self.browser_session.find_element_by_xpath(self.logout_fb[1]).click()
                self.time_patterns()
                self.browser_session.find_element_by_xpath(self.logout_fb[2]).click()
                return 1
            except:
                return -1
    ...
    def time_patterns(self, tp = None):
        ...
        #Also takes argument as second, if not given then it will take by default seconds to wait
        #Sucessful operation will return 1
    ...
    def page_navigation_partial(self, pg_name):
        #This function is use to load only specified page, need to given page name
        #for navigating the page
        
        if self.browser_session == None:
            #if no browser session available then it will return -1
            return -1
        else:
            #it will return find specified text partially
            #sucessful operation will also increment the browser_visit by one
    ...
    def page_posting(self):
        ...
        if self.browser_session == None:
            #if no browser_session started then it will return -1
            return -1
        
        else:
            #These operations will be followed by time patterns function, and in the end also
            #use reverse_visit to reverse the page by one
            
            self.browser_session.find_element_by_xpath(self.fb_posting[0]).click()
            self.time_patterns()
            self.browser_session.find_element_by_xpath(self.fb_posting[1]).send_keys(Keys.ENTER, self.textContents)
            self.time_patterns()
            self.browser_session.find_element_by_xpath(self.fb_posting[2]).click()
            self.time_patterns()
    ...
    def credential_loads_using_json(self):
        ...
        #Will help to set credentials instead of manual
        try:
...
def soc_bot():
    bot = Social_bot() #Creating an Instance of Social bot object
    bot.initiate_chrome() #Calling the initiate chrome method to start the chrome
    bot.credential_loads_using_json() #For loading credentials and other things
    bot.text_posting_content_load() #For loading text based posting content
    bot.page_load() #for loading the facebook
    bot.do_login() #for login the website
    ...
    #Below statement will pause the script until you clear the above given instrunctions
    if ask_to_block_notif.upper() == "Y":
        bot.page_load(bot.page_ref) #will load the specified page
        bot.time_patterns() #for pausing the script for sometime
        
        #Iterate through Page Names
        for link in bot.fb_page_partial:
            bot.page_navigation_partial(link) #load given facebook page by their name
            bot.time_patterns() #pause the script for sometime
            bot.page_posting() #for posting the text based content
            print("[+] Posting Done on {}".format(link)) #Print Page Name on Screen
        
        bot.do_logout() #for logging out the facebook
        bot.close_session() #for closing chrome & session
